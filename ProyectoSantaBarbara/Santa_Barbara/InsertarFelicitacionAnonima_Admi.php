<?php
  ...
  $fecha = $_POST['FechaFelicitacionAnonima'];
  $destinatario = $_POST['DestinatarioFelicitacionAnonima'];
  $descripcion = $_POST['DescripcionFelicitacionAnonima'];
  $estado = "En proceso";
  $codigo = alfanumerico();
  $tipo = "Felicitacion anónima";
 ...
 // FIN DE CODIGO NUEVO PARA MULTIPLES ARCHIVOS
  ...
  if($fecha != "" && $destinatario != "" && $descripcion != ""){
...
//
  $sql2 = "INSERT INTO felicitacionanonima (Fecha, Destinatario, Descripcion, Estado, CodAlfanumerico, ArchivoAdjunto, Tipo, SitioCreacion) 
  VALUES ('$fecha', '$destinatario', '$descripcion', '$estado', '$codigo', '$adjunto', '$tipo', '$sitioCreacion')";
...
echo '<script language="javascript">alert("Solicitud enviada con exito \nPor favor guarde este codigo para consultar por el estado de su solicitud \nCodigo: '.$codigo.'");</script>';
 // echo "$sql2"; //aqui conecta con la tabla felicitacionanonima
  ...
  //Aqui comienza la alerta de envio exitoso + el codigo alfanumerico para que el usuario lo pueda apuntar y guardarlo
  alerta();
  //Aqui termina la alerta
  }else{
    echo "Error";
  }
